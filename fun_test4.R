...
etree <- function(response,
  ...
  # Distances for each expansions' component
  if(split.type == 'coeff'){
    newcov.distance <- lapply(newcovariates,
                              function(nc){
                                nc <- as.matrix(nc)
                                lapply(1:ncol(nc),
                                       function(comp){
                                         compute.dissimilarity(nc[,comp])
                                       })
                              })
    covariates.large$newdist <- newcov.distance
  }
...
growtree <- function(id = 1L,
  ...
  for (kidid in 1:length(kids)) {
    ...
    covariates.updated$cov <- lapply(covariates$cov, function(c) subset(c, as.logical(w)))
    covariates.updated$newcov <- lapply(covariates$newcov, function(nc) subset(nc, as.logical(w)))
    covariates.updated$dist <- lapply(covariates$dist, function(d) subset(d, subset = as.logical(w), select = which(w == 1)))
    covariates.updated$newdist <- lapply(covariates$newdist,
                                         function(comp.dist){
                                           lapply(comp.dist,
                                                  function(nd) subset(nd, subset = as.logical(w), select = which(w == 1)))
                                         } )
...
findsplit <- function(response,
  ...
  if(split.type == 'coeff'){
    newxdist <- covariates$newdist[[xselect]]
  }
  ...
  switch(class(x),
         ...
         fdata      = {
           ...
           if(split.type == 'coeff'){
             ...
             p1 <- sapply(newxdist,
                          function(comp.dist) {
                            #set.seed(12345)
                            ct <- energy::dcor.test(comp.dist, compute.dissimilarity(y), R = R)
                            if (!is.na(ct$statistic)) {
                              return(c(ct$statistic, ct$p.value))
                            } else{
                              c(NA, NA)
         ...
         list = if(FALSE){
           ...
           if(split.type == 'coeff'){
             ...
             clindex <- cluster::pam(xdist, k = 2, diss = TRUE,
                                     cluster.only = TRUE)
             lev = levels(newx)
             splitindex = rep(NA, length(lev))
             splitindex[lev %in% newx[clindex==1]]<- 1
