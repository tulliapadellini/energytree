...
for(i in 1:M){
  ...
  class <- data_tot$class[order(data_tot$class)]
  dataf <- data.frame(class)
  V1 <- data_tot$V1[order(data_tot$class)]
  dat <- list("df" = dataf, "x" = V1)
  v1b <- create.fdata.basis(V1,l=1:15)
  a3 <- classif.glm(class~x, data = dat,
                    basis.b = list(V1 = v1b))
  f.test=fdata(test[,2:129])[order(test$cls)]
  newdat=list("x"=f.test)
  p1 = round( 
    predict.classif(a3, newdat, type = "probs")$prob.group,
    2) ##solo NA!!!
  c1=factor(colnames(p1)[apply(p1, 1, which.max)])
  t3 <- table(c1, test$cls)
  acc_m2[i] <- sum(diag(t3))/(nrow(test))
