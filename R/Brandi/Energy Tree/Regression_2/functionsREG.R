...
mytree <- function(Y, data, weights = NULL, 
  ...
  for(j in (n.var+1)){
    foo <- datanew[[j]]$coef
    colnames(foo) <- paste(names(data)[j-1], colnames(datanew[[j]]$coef), sep = ".")
...
findsplit <- function( response, data, weights, 
  ...
  p <- mytestREG(x=data[[2]]$fdata.est[which(weights==1)],y=y,R=R)
  
  # Bonferroni-adjusted p-value small enough?
  if ( is.na(p[2]) )  return(NULL)
