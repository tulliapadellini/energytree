...
mytree <- function(Y, data, weights = NULL, 
                   minbucket = 1, 
                   alpha = 0.05, R = 1000, 
  ...
  # growtree
  nodes <- growtree( id = 1L, response=datanew$Y, data = datanew, weights, 
                     minbucket = minbucket, 
                     alpha = alpha, R = R, 
  ...
  m.coef <- c()
  for(j in (n.var)){
    if(class(datanew[[j]])=="numeric") break
    if(class(datanew[j])=="list"){
    foo <- datanew[[j]]$coef
  ...
  for (j in n.var){
    if(class(datanew[[j]])!="list") x.num[j] <- 1
  }
  x.num <- which(!is.na(x.num))
  
  data3 <- m.coef
  for( i in x.num){
  data3=cbind(data3, datanew[[i]])
  }
  data3 <- data.frame(data3)
  
  data1=cbind(response, data3)
  
  fitted <- fitted_node(nodes, data = data3)
  formula=response~.
  names(data3)[(nb+1):length(names(data3))] <- names(data)[x.num]
  # return rich constparty object
  ret <- party(nodes, data = data.frame(data3), 
...
findsplit <- function( response, data, weights, 
  ...
  for(i in xselect){
    if(class(data[[i]])=="list"){
     p[(i-1),] <- mytestREG(x=data[[i]]$fdata.est[which(weights==1)],y=y,R=R)
    }else{
     p[(i-1),] <- mytestREG(x=data[[i]][which(weights==1)],y=y,R=R)
    }
  }
  ...
  if(is.numeric(x)){
    return( list(sp=partysplit( varid = as.integer(xselect+nb-2),
                        breaks = splitindex,
