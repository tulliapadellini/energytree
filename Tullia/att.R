...
#' @param covariates list of covariates, each element of the list is a different variable (**TO ADD: CHECK CONSISTENCY OF NOBS AND DIMENSIONS**)
#' @param case.weights
#' @param minbucket
#' @param alpha significance level for the global test of association and, if \code{coef.split.type = "test"} for the test used in the split
#' @param R
#' @param split.type type of the split when covariates are "complex" (i.e. they are not numeric). This variable can be se to
#'  either "coeff" (only available when there is a coeffient representation) or "cluster"
#' @param coef.split.type either "variance" or "test"
#' @param nb number of basis elements (for fdata covariates only)
#'
#' @export
#'
#' @examples
...
etree <- function(response,
  ...
  fitted.obs <- fitted_node(nodes, data =df)

  # Returning a rich constparty object
  data1 = cbind('response' = as.data.frame(response), matrix(rep(1:length(newcovariates.df), each = length(response)), ncol =length(newcovariates.df), byrow = F ))
  ...
  ret <- party(nodes,
               ...
               fitted = data.frame("(fitted)" = fitted.obs,
                                   ...
                                   check.names = FALSE),
               terms = terms(response ~ ., data = data.frame(response, covariates)))
